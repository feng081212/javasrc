...
public class TccTransactionManager {
    ...
    public void cancel(TccTransaction currentTransaction) {
        ...
        if (currentTransaction.getStatus() == TccActionEnum.TRYING.getCode() && Objects.equals(currentTransaction.getPattern(), TccPatternEnum.CC.getCode())) {
    ...
    private void executeHandler(boolean success, final TccTransaction currentTransaction,List<Participant> failList) {
        if (success) {
    ...
    private List<Participant> filterPoint(TccTransaction currentTransaction) {
        ...
        if (CollectionUtils.isNotEmpty(participants)) {
            ...
            if (currentTransaction.getStatus() == TccActionEnum.TRYING.getCode() && currentTransaction.getRole() == TccRoleEnum.START.getCode()) {
                return participants.stream().limit(participants.size()).filter(Objects::nonNull).collect(Collectors.toList());
            }
        }
        return participants;
    }
