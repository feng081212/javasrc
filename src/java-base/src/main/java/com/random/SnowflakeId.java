...
public class SnowflakeId {
    ...
    /** 生成毫秒内序列的掩码，这里为4095 (0b111111111111=0xfff=4095) */
     ...
     * 解密
     * @param snowflakeId
     * @return 返回Long数组，数组长度是4 <br>数组中每个元素依次代表：时间戳、数据中心ID、工作ID、毫秒内随机数
     */
    public Long[] decode(long snowflakeId){
    	long sequence  = snowflakeId & 0xfff;
        long workerId = (snowflakeId >> WORKER_ID_SHIFT)  & 0x1f ;
        long datacenterId = (snowflakeId >> DATA_CENTER_ID_SHIFT ) & 0x1f ;
        long time = (snowflakeId >> TIMESTAMP_SHIFT) + START_TIME;
        return new Long[]{time,datacenterId,workerId,sequence} ;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(idWorker.decode(id));
    }
}